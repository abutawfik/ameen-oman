...
const ApiSection = () => {
  ...
  const features = [
    { key: "auth", icon: "ri-key-line", color: "#22D3EE" },
  ...
  return (
    <section id="api-integration" className="py-20 md:py-28 relative" style={{ background: "#060D1A" }}>
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-4 md:px-8">
        ...
        <div className="text-center mb-14">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-cyan-500/30 bg-cyan-500/5 mb-4">
            <span className="text-cyan-400 text-xs font-['JetBrains_Mono'] tracking-widest uppercase">B2B INTEGRATION</span>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
          {/* B2B API */}
          <div className="rounded-xl p-6 border" style={{ background: "rgba(10,22,40,0.8)", borderColor: "rgba(34,211,238,0.2)", backdropFilter: "blur(12px)" }}>
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 flex items-center justify-center rounded-lg" style={{ background: "rgba(34,211,238,0.1)", border: "1px solid rgba(34,211,238,0.3)" }}>
                <i className="ri-code-s-slash-line text-cyan-400 text-lg" />
              </div>
            ...
            <div className="space-y-2">
              ...
              {apiEndpoints.map((ep, i) => (
                <div key={i} className="flex items-center gap-3 p-2.5 rounded-lg" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.05)" }}>
                  <span className={`text-xs font-bold font-['JetBrains_Mono'] px-2 py-0.5 rounded ${ep.method === "GET" ? "text-green-400 bg-green-400/10" : "text-cyan-400 bg-cyan-400/10"}`}>{ep.method}</span>
          ...
          <div className="flex flex-col gap-6">
            ...
            {/* Integration Steps */}
            <div className="rounded-xl p-6 border" style={{ background: "rgba(10,22,40,0.8)", borderColor: "rgba(34,211,238,0.15)", backdropFilter: "blur(12px)" }}>
              ...
              <div className="space-y-3">
                {integrationSteps.map((s) => (
                  <div key={s.step} className="flex items-start gap-3">
                    <div className="w-7 h-7 flex items-center justify-center rounded-full bg-cyan-400/10 border border-cyan-400/30 flex-shrink-0 mt-0.5">
                      <span className="text-cyan-400 text-xs font-bold font-['JetBrains_Mono']">{s.step}</span>
                    </div>
                    <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-10">
          {features.map((f) => (
            <div key={f.key} className="flex flex-col items-center text-center p-4 rounded-xl border" style={{ background: "rgba(10,22,40,0.6)", borderColor: "rgba(255,255,255,0.06)" }}>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          <button className="flex items-center gap-2 px-6 py-3 bg-cyan-400 text-[#060D1A] font-semibold rounded-md hover:bg-cyan-300 transition-colors duration-200 whitespace-nowrap cursor-pointer text-sm">
            <i className="ri-send-plane-line" />
            {t("api.requestAccess")}
          </button>
          <button className="flex items-center gap-2 px-6 py-3 border border-cyan-500/50 text-cyan-400 font-semibold rounded-md hover:bg-cyan-500/10 transition-colors duration-200 whitespace-nowrap cursor-pointer text-sm">
