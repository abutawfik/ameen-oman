...
const AboutSection = () => {
  ...
  return (
    <section id="about" className="py-20 md:py-28 relative" style={{ background: "#060D1A" }}>
      <div className="absolute inset-0 opacity-10" style={{ backgroundImage: `linear-gradient(rgba(34,211,238,0.1) 1px, transparent 1px), linear-gradient(90deg, rgba(34,211,238,0.1) 1px, transparent 1px)`, backgroundSize: "80px 80px" }} />
      <div className="relative z-10 max-w-[1400px] mx-auto px-4 md:px-8">
        <div className="text-center mb-14">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-cyan-500/30 bg-cyan-500/5 mb-4">
            <span className="text-cyan-400 text-xs font-['JetBrains_Mono'] tracking-widest uppercase">ABOUT AMEEN</span>
          </div>
          <h2 className="text-3xl md:text-4xl font-bold text-white font-['Inter'] mb-2">{t("about.title")}</h2>
          <div className="w-16 h-0.5 bg-cyan-400 mx-auto mt-4 mb-4" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          <div className="flex flex-col gap-5">
            {[
              { titleKey: "about.ropTitle", descKey: "about.ropDesc", icon: "ri-shield-star-line", color: "#22D3EE" },
              ...
              <div key={card.titleKey} className="flex gap-4 p-5 rounded-xl border" style={{ background: "rgba(10,22,40,0.8)", borderColor: "rgba(34,211,238,0.12)", backdropFilter: "blur(12px)" }}>
            ...
            {/* Contact info */}
            <div className="p-5 rounded-xl border" style={{ background: "rgba(10,22,40,0.8)", borderColor: "rgba(34,211,238,0.12)" }}>
              ...
              <div className="space-y-2">
                {[
                  { icon: "ri-phone-line", text: t("footer.phone"), color: "#22D3EE" },
          ...
          {/* Contact Form */}
          <div className="p-6 rounded-xl border" style={{ background: "rgba(10,22,40,0.8)", borderColor: "rgba(34,211,238,0.2)", backdropFilter: "blur(12px)" }}>
            <h3 className="text-white font-bold text-lg font-['Inter'] mb-5 flex items-center gap-2">
              <i className="ri-mail-send-line text-cyan-400" />
              ...
              <form data-readdy-form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input name="name" required placeholder={t("about.form.namePlaceholder")} className="w-full px-3 py-2.5 rounded-lg text-sm text-white placeholder-gray-600 outline-none focus:border-cyan-400/60 transition-colors font-['Inter']" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)" }} />
                  </div>
                  <div>
                    <label className="block text-gray-400 text-xs mb-1.5 font-['Inter']">{t("about.form.email")}</label>
                    <input name="email" type="email" required placeholder={t("about.form.emailPlaceholder")} className="w-full px-3 py-2.5 rounded-lg text-sm text-white placeholder-gray-600 outline-none focus:border-cyan-400/60 transition-colors font-['Inter']" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)" }} />
                  </div>
                </div>
                <div>
                ...
                <div>
                  ...
                  <select name="subject" className="w-full px-3 py-2.5 rounded-lg text-sm text-white outline-none font-['Inter'] cursor-pointer" style={{ background: "rgba(10,22,40,0.95)", border: "1px solid rgba(255,255,255,0.1)" }}>
                ...
                <button type="submit" disabled={status === "sending"} className="w-full flex items-center justify-center gap-2 py-3 bg-cyan-400 text-[#060D1A] font-semibold rounded-md hover:bg-cyan-300 transition-colors duration-200 cursor-pointer text-sm font-['Inter'] disabled:opacity-60">
