...
const SocialIntelligencePage = () => {
  ...
  const TABS: { id: Tab; icon: string; label: string; labelAr: string; badge?: string; badgeColor?: string }[] = [
    ...
    { id: "lookup",    icon: "ri-search-eye-line",     label: "Phone Lookup",      labelAr: "بحث الهاتف",      badge: "Tier 1", badgeColor: "#D4A84B" },
      ...
      <header className="sticky top-0 z-40 flex items-center justify-between px-6 py-3 border-b"
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="hidden xl:flex items-center gap-2">
            {[
              { label: "Tier 1: Phone Lookup", color: "#D4A84B" },
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div className="rounded-2xl border p-6" style={{ background: "rgba(20,29,46,0.8)", borderColor: "rgba(181,142,60,0.2)", backdropFilter: "blur(12px)" }}>
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-10 h-10 flex items-center justify-center rounded-xl" style={{ background: "rgba(181,142,60,0.1)", border: "1px solid rgba(181,142,60,0.25)" }}>
                    <i className="ri-search-eye-line text-gold-400 text-lg" />
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      <h3 className="text-white font-bold">{isAr ? "Tier 1 — بحث الهوية الاجتماعية" : "Tier 1 — Social Identity Lookup"}</h3>
                      <span className="px-2 py-0.5 rounded-full text-xs font-bold" style={{ background: "rgba(181,142,60,0.12)", color: "#D4A84B", fontSize: "9px" }}>PHONE → SOCIAL</span>
                    </div>
                ...
                <div className="space-y-3">
                  {[
                    ...
                    <div key={item.label} className="flex items-start gap-3 px-3 py-2 rounded-xl" style={{ background: "rgba(181,142,60,0.04)", border: "1px solid rgba(181,142,60,0.1)" }}>
                      <div className="w-6 h-6 flex items-center justify-center rounded-lg flex-shrink-0" style={{ background: "rgba(181,142,60,0.1)" }}>
                        <i className={`${item.icon} text-gold-400 text-xs`} />
                      </div>
                      <div>
                        <p className="text-gold-400 text-xs font-bold">{item.label}</p>
              ...
              {/* Tier 2 */}
              <div className="rounded-2xl border p-6" style={{ background: "rgba(20,29,46,0.8)", borderColor: "rgba(250,204,21,0.2)", backdropFilter: "blur(12px)" }}>
                <div className="flex items-center gap-3 mb-4">
                ...
                <div className="space-y-3">
                  {[
                    ...
                    { icon: "ri-emotion-line", color: "#D4A84B", label: isAr ? "تحليل المشاعر" : "Sentiment Analysis", desc: isAr ? "إيجابي / محايد / سلبي / تهديد — لكل منشور مُبلَّغ" : "Positive / Neutral / Negative / Threat — per flagged post" },
                  ].map((item) => (
            ...
            <div className="rounded-2xl border p-5" style={{ background: "rgba(20,29,46,0.8)", borderColor: "rgba(181,142,60,0.12)", backdropFilter: "blur(12px)" }}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {[
                  { icon: "ri-map-pin-line", color: "#D4A84B", title: isAr ? "تأكيد الموقع" : "Location Confirmation", desc: isAr ? "منشور من مسقط + SIM مسجّل في نزوى = تناقض → تحقيق" : "Post from Muscat + SIM registered in Nizwa = contradiction → investigate" },
            ...
            <div className="rounded-2xl border p-5" style={{ background: "rgba(20,29,46,0.8)", borderColor: "rgba(181,142,60,0.12)", backdropFilter: "blur(12px)" }}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                {[
                  { code: "AMN-OSI-20260405-0091", label: isAr ? "بحث هاتف — Reza Tehrani" : "Phone Lookup — Reza Tehrani", color: "#D4A84B" },
