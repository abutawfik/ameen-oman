...
const STREAMS = [
  ...
  { id: "health",      num: "07", name: "Health Interactions",      nameAr: "التفاعلات الصحية",           icon: "ri-heart-pulse-line",     color: "#C94A5E", score: 89, violations: 6,  entities: 34,  eventsToday: 1847, trend: [84,85,86,87,88,88,89], status: "compliant" },
  ...
  { id: "transport",   num: "09", name: "Public Transport",         nameAr: "النقل العام",                icon: "ri-bus-line",             color: "#C98A1B", score: 93, violations: 4,  entities: 5,   eventsToday: 42891,trend: [88,89,90,91,92,92,93], status: "compliant" },
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <main className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              { label: "Overall Score", labelAr: "الدرجة الإجمالية", value: `${avgScore}%`, color: avgScore >= 90 ? "#4ADE80" : avgScore >= 75 ? "#FACC15" : "#C94A5E", icon: "ri-award-line", sub: "Across 16 streams" },
              ...
              { label: "Total Violations", labelAr: "إجمالي المخالفات", value: totalViolations, color: "#C98A1B", icon: "ri-error-warning-line", sub: "This month" },
              { label: "Critical Streams", labelAr: "مصادر حرجة", value: criticalCount, color: "#C94A5E", icon: "ri-alarm-warning-line", sub: "Require immediate action" },
            ].map((kpi) => (
              <div key={kpi.label} className="p-4 rounded-xl relative overflow-hidden" style={{ background: "rgba(10,37,64,0.8)", border: `1px solid ${kpi.color}20` }}>
            ...
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-4 gap-3">
                {[
                  ...
                  { label: "Critical", labelAr: "حرج", count: criticalCount, color: "#C94A5E", pct: (criticalCount / 16) * 100 },
                  { label: "Open Violations", labelAr: "مخالفات مفتوحة", count: VIOLATIONS.filter((v) => v.status === "open" || v.status === "escalated").length, color: "#C98A1B", pct: 0 },
                ].map((s) => (
              ...
              <div className="rounded-xl overflow-hidden" style={{ background: "rgba(10,37,64,0.8)", border: "1px solid rgba(184,138,60,0.1)" }}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x" style={{ borderColor: "rgba(184,138,60,0.05)" }}>
                  {STREAMS.filter((s) => statusFilter === "all" || s.status === statusFilter).map((stream, i) => {
                    ...
                    return (
                      <div key={stream.id} className="flex items-center gap-3 px-4 py-3 transition-colors cursor-pointer"
                        style={{ borderBottom: "1px solid rgba(184,138,60,0.04)" }}
                        onMouseEnter={(e) => (e.currentTarget.style.background = "rgba(184,138,60,0.03)")}
                        ...
                        {stream.violations > 0 && (
                          <div className="w-5 h-5 flex items-center justify-center rounded-full flex-shrink-0"
                            style={{ background: "rgba(201,138,27,0.15)", border: "1px solid rgba(201,138,27,0.3)" }}>
                ...
                {filteredStreams.map((stream) => {
                  ...
                  return (
                    <button key={stream.id} onClick={() => setSelectedStream(stream)}
                      ...
                      style={{
                        background: selectedStream.id === stream.id ? "rgba(184,138,60,0.08)" : "rgba(10,37,64,0.6)",
                        border: `1px solid ${selectedStream.id === stream.id ? "rgba(184,138,60,0.25)" : "rgba(255,255,255,0.05)"}`,
                      }}>
              ...
              <div className="flex-1 space-y-4">
                {(() => {
                  ...
                  return (
                    <>
                      {/* Header card */}
                      <div className="p-5 rounded-xl" style={{ background: "rgba(10,37,64,0.8)", border: `1px solid ${cfg.color}25` }}>
                          ...
                          {s.trend.map((val, i) => (
                            <div key={i} className="flex-1 flex flex-col items-center gap-1">
                              ...
                              <div className="w-full rounded-t-sm transition-all"
                                style={{
                                  height: `${(val / 100) * 100}%`,
                                  background: val >= 90 ? "rgba(74,222,128,0.7)" : val >= 75 ? "rgba(250,204,21,0.7)" : "rgba(201,74,94,0.7)",
                                  minHeight: "4px",
                                }} />
          ...
          {mainTab === "violations" && (
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                {VIOLATIONS.map((v) => {
                  const sev = severityConfig[v.severity];
                  const statusColor = v.status === "resolved" ? "#4ADE80" : v.status === "escalated" ? "#C94A5E" : v.status === "investigating" ? "#FACC15" : "#9CA3AF";
                  return (
                    <div key={v.id} className="p-4 rounded-xl" style={{ background: "rgba(10,37,64,0.8)", border: `1px solid ${sev.color}20`, borderLeft: `3px solid ${sev.color}` }}>
                    ...
                    return (
                      <div key={month} className="flex-1 flex flex-col items-center gap-1">
                        ...
                        <div className="w-full rounded-t-sm"
                          style={{
                            height: `${avg}%`,
                            background: avg >= 90 ? "rgba(74,222,128,0.7)" : avg >= 80 ? "rgba(250,204,21,0.7)" : "rgba(201,74,94,0.7)",
                            minHeight: "4px",
                          }} />
                      ...
                      {STREAMS.map((stream, i) => {
                        ...
                        return (
                          <tr key={stream.id} style={{ borderBottom: i < STREAMS.length - 1 ? "1px solid rgba(255,255,255,0.03)" : "none" }}
                            onMouseEnter={(e) => (e.currentTarget.style.background = "rgba(184,138,60,0.02)")}
                            onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")}>
                            ...
                            {stream.trend.map((val, ti) => (
                              <td key={ti} className="px-3 py-2 text-center">
                                <span className="text-xs font-['JetBrains_Mono']"
                                  style={{ color: val >= 90 ? "#4ADE80" : val >= 75 ? "#FACC15" : "#C94A5E" }}>
                                  {val}
                                </span>
                              </td>
                            ))}
                            <td className="px-4 py-2 text-center">
                              <span className="text-xs font-bold font-['JetBrains_Mono']" style={{ color: change >= 0 ? "#4ADE80" : "#C94A5E" }}>
                                {change >= 0 ? "+" : ""}{change}
                              </span>
                            </td>
                            <td className="px-4 py-2 text-center">
