...
const STREAMS = [
  { id: "hotel",       num: "01", name: "Hotel Intelligence",       nameAr: "الاستخبارات الفندقية",       icon: "ri-hotel-line",           color: "#D4A84B", score: 96, violations: 2,  entities: 47,  eventsToday: 1284, trend: [88,90,92,93,94,95,96], status: "compliant" },
  { id: "car-rental",  num: "02", name: "Car Rental Monitoring",    nameAr: "مراقبة تأجير السيارات",      icon: "ri-car-line",             color: "#D4A84B", score: 94, violations: 3,  entities: 23,  eventsToday: 2103, trend: [89,90,91,92,93,93,94], status: "compliant" },
  ...
  { id: "marine",      num: "15", name: "Marine & Maritime",        nameAr: "البحرية والملاحة",           icon: "ri-anchor-line",          color: "#D4A84B", score: 71, violations: 28, entities: 14,  eventsToday: 892,  trend: [60,62,64,66,68,70,71], status: "critical" },
...
const VIOLATIONS = [
  ...
  { id: "VIO-2026-0440", stream: "Marine & Maritime", streamIcon: "ri-anchor-line", streamColor: "#D4A84B", entity: "Muscat Marina Authority", type: "Missing Fields", severity: "critical", date: "2026-04-06", detail: "Vessel registration events missing IMO number and flag state — 89 records", status: "open" },
  ...
  { id: "VIO-2026-0428", stream: "Marine & Maritime", streamIcon: "ri-anchor-line", streamColor: "#D4A84B", entity: "Khasab Port Authority", type: "Late Submission", severity: "high", date: "2026-04-04", detail: "Docking events delayed 48 hours — 156 vessel movements not reported in time", status: "open" },
  ...
  return (
    <div className="flex flex-col h-full" style={{ background: "#0B1220", direction: isAr ? "rtl" : "ltr" }}>
      <div className="fixed inset-0 pointer-events-none"
        style={{ backgroundImage: "linear-gradient(rgba(181,142,60,0.025) 1px, transparent 1px), linear-gradient(90deg, rgba(181,142,60,0.025) 1px, transparent 1px)", backgroundSize: "40px 40px" }} />
        ...
        <main className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              ...
              <div key={kpi.label} className="p-4 rounded-xl relative overflow-hidden" style={{ background: "rgba(20,29,46,0.8)", border: `1px solid ${kpi.color}20` }}>
          ...
          <div className="flex gap-1 p-1 rounded-xl" style={{ background: "rgba(20,29,46,0.8)", border: "1px solid rgba(181,142,60,0.1)" }}>
            {tabs.map((tab) => (
              <button key={tab.id} onClick={() => setMainTab(tab.id)}
                ...
                style={{
                  background: mainTab === tab.id ? "rgba(181,142,60,0.12)" : "transparent",
                  color: mainTab === tab.id ? "#D4A84B" : "#6B7280",
                  border: mainTab === tab.id ? "1px solid rgba(181,142,60,0.25)" : "1px solid transparent",
                }}>
            ...
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-4 gap-3">
                {[
                  ...
                  <div key={s.label} className="p-4 rounded-xl" style={{ background: "rgba(20,29,46,0.8)", border: `1px solid ${s.color}20` }}>
              ...
              <div className="rounded-xl overflow-hidden" style={{ background: "rgba(20,29,46,0.8)", border: "1px solid rgba(181,142,60,0.1)" }}>
                <div className="px-5 py-3 border-b flex items-center justify-between" style={{ borderColor: "rgba(181,142,60,0.08)" }}>
                  ...
                  <div className="flex gap-1">
                    {["all", "compliant", "warning", "critical"].map((f) => (
                      <button key={f} onClick={() => setStatusFilter(f)}
                        ...
                        style={{
                          background: statusFilter === f ? "rgba(181,142,60,0.12)" : "transparent",
                          color: statusFilter === f ? "#D4A84B" : "#6B7280",
                          border: `1px solid ${statusFilter === f ? "rgba(181,142,60,0.25)" : "transparent"}`,
                        }}>
                        {f}
                      </button>
                    ))}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x" style={{ borderColor: "rgba(181,142,60,0.05)" }}>
                  {STREAMS.filter((s) => statusFilter === "all" || s.status === statusFilter).map((stream, i) => {
                    ...
                    return (
                      <div key={stream.id} className="flex items-center gap-3 px-4 py-3 transition-colors cursor-pointer"
                        style={{ borderBottom: "1px solid rgba(181,142,60,0.04)" }}
                ...
                <div className="flex gap-1 mb-2">
                  {(["score", "violations", "events"] as const).map((s) => (
                    <button key={s} onClick={() => setSortBy(s)}
                      ...
                      style={{
                        background: sortBy === s ? "rgba(181,142,60,0.12)" : "rgba(20,29,46,0.6)",
                        color: sortBy === s ? "#D4A84B" : "#6B7280",
                        border: `1px solid ${sortBy === s ? "rgba(181,142,60,0.25)" : "rgba(255,255,255,0.05)"}`,
                      }}>
                      {s}
                    </button>
                  ))}
                ...
                {filteredStreams.map((stream) => {
                  ...
                  return (
                    <button key={stream.id} onClick={() => setSelectedStream(stream)}
                      ...
                      style={{
                        background: selectedStream.id === stream.id ? "rgba(181,142,60,0.08)" : "rgba(20,29,46,0.6)",
                        border: `1px solid ${selectedStream.id === stream.id ? "rgba(181,142,60,0.25)" : "rgba(255,255,255,0.05)"}`,
                      }}>
              ...
              <div className="flex-1 space-y-4">
                {(() => {
                  ...
                  return (
                    <>
                      {/* Header card */}
                      <div className="p-5 rounded-xl" style={{ background: "rgba(20,29,46,0.8)", border: `1px solid ${cfg.color}25` }}>
                        ...
                        <div className="grid grid-cols-4 gap-3">
                          {[
                            { label: "Entities", value: s.entities, color: "#D4A84B" },
                            ...
                            <div key={stat.label} className="p-3 rounded-lg text-center" style={{ background: "rgba(11,18,32,0.6)", border: `1px solid ${stat.color}15` }}>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                {VIOLATIONS.map((v) => {
                  ...
                  return (
                    <div key={v.id} className="p-4 rounded-xl" style={{ background: "rgba(20,29,46,0.8)", border: `1px solid ${sev.color}20`, borderLeft: `3px solid ${sev.color}` }}>
          ...
          {mainTab === "trends" && (
            <div className="space-y-5">
              ...
              <div className="rounded-xl overflow-hidden" style={{ background: "rgba(20,29,46,0.8)", border: "1px solid rgba(181,142,60,0.1)" }}>
                <div className="px-5 py-3 border-b" style={{ borderColor: "rgba(181,142,60,0.08)" }}>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead>
                      <tr style={{ borderBottom: "1px solid rgba(181,142,60,0.06)" }}>
                      ...
                      {STREAMS.map((stream, i) => {
                        ...
                        return (
                          <tr key={stream.id} style={{ borderBottom: i < STREAMS.length - 1 ? "1px solid rgba(255,255,255,0.03)" : "none" }}
                            onMouseEnter={(e) => (e.currentTarget.style.background = "rgba(181,142,60,0.02)")}
                            onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")}>
