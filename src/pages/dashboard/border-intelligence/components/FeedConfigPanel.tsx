...
const SOURCE_SYSTEMS: SourceSystem[] = [
  { id: "iborders-air", name: "iBorders — Air", nameAr: "iBorders — جوي", type: "Land/Sea/Air Border Control", typeAr: "مراقبة الحدود البرية/البحرية/الجوية", icon: "ri-flight-takeoff-line", status: "online", lastSync: "2026-04-05 14:32:07", eventsToday: 4821, color: "#22D3EE" },
...
const EVENT_TYPES: EventTypeRow[] = [
  ...
  { code: "EXIT", label: "Exit (Departure)", labelAr: "خروج (مغادرة)", count: 2987, color: "#22D3EE", icon: "ri-logout-box-line", enabled: true },
  ...
  { code: "REENTRY", label: "Re-Entry", labelAr: "إعادة دخول", count: 198, color: "#22D3EE", icon: "ri-refresh-line", enabled: true },
];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: isAr ? "إجمالي الأحداث اليوم" : "Total Events Today", value: totalToday.toLocaleString(), icon: "ri-pulse-line", color: "#22D3EE" },
          ...
          {SOURCE_SYSTEMS.map((sys) => (
            <div key={sys.id} className="flex items-center gap-4 px-6 py-4 hover:bg-white/[0.02] transition-colors">
              ...
              <div className="hidden lg:block text-right">
                ...
                <div className="text-cyan-400 text-xs font-['JetBrains_Mono']">{sys.lastSync.split(" ")[1]}</div>
              </div>
      ...
      <div className="rounded-2xl border overflow-hidden" style={{ background: "rgba(10,22,40,0.8)", borderColor: "rgba(34,211,238,0.15)", backdropFilter: "blur(12px)" }}>
        <div className="flex items-center justify-between px-6 py-4 border-b" style={{ borderColor: "rgba(34,211,238,0.1)" }}>
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 flex items-center justify-center rounded-lg" style={{ background: "rgba(34,211,238,0.1)", border: "1px solid rgba(34,211,238,0.2)" }}>
              <i className="ri-list-settings-line text-cyan-400 text-sm" />
            </div>
            <div>
          ...
          {eventTypes.map((evt) => (
            <div key={evt.code} className="flex items-center gap-4 px-6 py-3.5 hover:bg-white/[0.02] transition-colors">
              ...
              <button type="button" onClick={() => toggleEvent(evt.code)}
                ...
                style={{ background: evt.enabled ? "#22D3EE" : "rgba(255,255,255,0.1)" }}>
      ...
      <div className="rounded-2xl border p-6" style={{ background: "rgba(10,22,40,0.8)", borderColor: "rgba(34,211,238,0.15)", backdropFilter: "blur(12px)" }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {[
            { icon: "ri-id-card-line", label: isAr ? "وثيقة الشخص + النوع + الجنسية" : "Person Document + Type + Nationality", color: "#22D3EE" },
            ...
            { icon: "ri-ticket-line", label: isAr ? "مرجع الحجز + المقعد/الكابينة" : "Booking Reference + Seat/Cabin", color: "#22D3EE" },
