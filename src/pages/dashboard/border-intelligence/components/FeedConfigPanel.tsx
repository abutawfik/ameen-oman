...
const SOURCE_SYSTEMS: SourceSystem[] = [
  { id: "iborders-air", name: "iBorders — Air", nameAr: "iBorders — جوي", type: "Land/Sea/Air Border Control", typeAr: "مراقبة الحدود البرية/البحرية/الجوية", icon: "ri-flight-takeoff-line", status: "online", lastSync: "2026-04-05 14:32:07", eventsToday: 4821, color: "#D4A84B" },
  ...
  { id: "iborders-sea", name: "iBorders — Sea", nameAr: "iBorders — بحري", type: "Sea Ports & Vessels", typeAr: "الموانئ البحرية والسفن", icon: "ri-ship-line", status: "online", lastSync: "2026-04-05 14:30:12", eventsToday: 389, color: "#FB923C" },
...
const EVENT_TYPES: EventTypeRow[] = [
  ...
  { code: "EXIT", label: "Exit (Departure)", labelAr: "خروج (مغادرة)", count: 2987, color: "#D4A84B", icon: "ri-logout-box-line", enabled: true },
  ...
  { code: "VISA_EXPIRED", label: "Visa Expired", labelAr: "تأشيرة منتهية", count: 94, color: "#FB923C", icon: "ri-calendar-close-line", enabled: true },
  { code: "OVERSTAY", label: "Overstay Detected", labelAr: "تجاوز مدة الإقامة", count: 31, color: "#F87171", icon: "ri-alarm-warning-line", enabled: true },
  { code: "DEPORTATION", label: "Deportation", labelAr: "ترحيل", count: 7, color: "#F87171", icon: "ri-user-unfollow-line", enabled: true },
  { code: "TRANSIT", label: "Transit", labelAr: "عبور", count: 512, color: "#9CA3AF", icon: "ri-route-line", enabled: true },
  { code: "REENTRY", label: "Re-Entry", labelAr: "إعادة دخول", count: 198, color: "#D4A84B", icon: "ri-refresh-line", enabled: true },
];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: isAr ? "إجمالي الأحداث اليوم" : "Total Events Today", value: totalToday.toLocaleString(), icon: "ri-pulse-line", color: "#D4A84B" },
          ...
          <div key={stat.label} className="rounded-xl border p-4" style={{ background: "rgba(20,29,46,0.8)", borderColor: "rgba(181,142,60,0.12)", backdropFilter: "blur(12px)" }}>
            ...
            <div key={sys.id} className="flex items-center gap-4 px-6 py-4 hover:bg-white/[0.02] transition-colors">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5">
                  ...
                  <span className="text-xs font-semibold" style={{ color: sys.status === "online" ? "#4ADE80" : "#F87171" }}>
          ...
          {eventTypes.map((evt) => (
            <div key={evt.code} className="flex items-center gap-4 px-6 py-3.5 hover:bg-white/[0.02] transition-colors">
              ...
              <button type="button" onClick={() => toggleEvent(evt.code)}
                ...
                style={{ background: evt.enabled ? "#D4A84B" : "rgba(255,255,255,0.1)" }}>
      ...
      <div className="rounded-2xl border p-6" style={{ background: "rgba(20,29,46,0.8)", borderColor: "rgba(181,142,60,0.15)", backdropFilter: "blur(12px)" }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {[
            { icon: "ri-id-card-line", label: isAr ? "وثيقة الشخص + النوع + الجنسية" : "Person Document + Type + Nationality", color: "#D4A84B" },
            ...
            { icon: "ri-flight-takeoff-line", label: isAr ? "الناقل (شركة طيران/سفينة)" : "Carrier (airline/vessel)", color: "#FB923C" },
            { icon: "ri-ticket-line", label: isAr ? "مرجع الحجز + المقعد/الكابينة" : "Booking Reference + Seat/Cabin", color: "#D4A84B" },
