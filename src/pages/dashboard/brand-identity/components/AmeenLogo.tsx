...
export const AmeenShield = ({ size = 64, light = false }: { size?: number; light?: boolean }) => {
  const cyan = "#22D3EE";
  const dark = light ? "#060D1A" : "#060D1A";
  ...
  return (
    <svg width={size} height={size * 1.1} viewBox="0 0 100 110" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <polygon
        ...
        fill={light ? "white" : "rgba(10,22,40,0.95)"}
        stroke={cyan}
        strokeWidth="2.5"
      />
      <polygon
      ...
      <polygon
        points="50,9 88,30 88,70 50,91 12,70 12,30"
        fill={light ? "rgba(34,211,238,0.05)" : "rgba(34,211,238,0.06)"}
      />
      ...
      {/* Pupil */}
      <circle cx="50" cy="55" r="1.2" fill={light ? "white" : "#060D1A"} />
...
export const HospitalityLeaf = ({ size = 16 }: { size?: number }) => (
  <svg width={size} height={size} viewBox="0 0 16 16" fill="none">
    <path d="M8 14 C4 10 2 6 4 3 C6 1 10 2 12 5 C14 8 12 12 8 14Z" fill="#22D3EE" opacity="0.7" />
    <path d="M8 14 L8 7" stroke="#22D3EE" strokeWidth="1" strokeLinecap="round" />
  </svg>
);
  ...
  if (variant === "light") {
    return (
      <div className={`flex flex-col items-center gap-2 ${className}`}>
        ...
        <div className="text-center">
          <div className="font-black tracking-widest uppercase" style={{ color: "#060D1A", fontSize: size * 0.22, fontFamily: "'Inter', sans-serif", letterSpacing: "0.18em" }}>
            AMEEN
          </div>
          <div className="font-bold" style={{ color: "#060D1A", fontSize: size * 0.14, fontFamily: "'Noto Kufi Arabic', 'Arial', sans-serif", opacity: 0.7 }}>
            أمين
          </div>
  ...
  if (variant === "cobranded") {
    return (
      <div className={`flex items-center gap-4 ${className}`}>
        ...
        {/* Divider */}
        <div style={{ width: 1, height: size * 0.8, background: "rgba(34,211,238,0.25)" }} />
        {/* AMEEN */}
