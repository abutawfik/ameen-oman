...
const EcommerceIntelligencePage = () => {
  ...
  const TABS: { id: Tab; icon: string; label: string; labelAr: string; badge?: string; badgeColor?: string }[] = [
    ...
    { id: "persons",    icon: "ri-user-search-line",          label: "Person Profiles",   labelAr: "ملفات الأشخاص",   badge: "3",  badgeColor: "#FB923C" },
    ...
    { id: "submit",     icon: "ri-add-circle-line",           label: "Record Event",       labelAr: "تسجيل حدث",       badge: "5",  badgeColor: "#D4A84B" },
  ];
  ...
  return (
    <div className="min-h-screen font-['Inter']" style={{ background: "#0B1220" }} dir={isAr ? "rtl" : "ltr"}>
      ...
      <div className="sticky top-[57px] z-30 flex items-center gap-1 px-6 py-2 border-b overflow-x-auto"
        style={{ background: "rgba(11,18,32,0.9)", borderColor: "rgba(181,142,60,0.08)", backdropFilter: "blur(12px)" }}>
        {TABS.map((tab) => (
          ...
          <>
            ...
            <div className="rounded-2xl border p-5" style={{ background: "rgba(20,29,46,0.8)", borderColor: "rgba(181,142,60,0.12)", backdropFilter: "blur(12px)" }}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {[
                  ...
                  { icon: "ri-map-pin-line", color: "#D4A84B", title: isAr ? "تحديد الموقع الفعلي" : "Actual Location Reveal", desc: isAr ? "الشراء من متجر في نزوى يُثبت الوجود الجسدي — يتعارض مع العنوان المُسجَّل" : "Purchase at Nizwa store proves physical presence — contradicts registered address" },
                  { icon: "ri-spy-line", color: "#F87171", title: isAr ? "كشف النوايا" : "Intent Detection", desc: isAr ? "شراء مشوش إشارة + شرائح SIM بالجملة + مواد كيميائية = نمط يستحق التحقيق" : "Signal jammer + bulk SIMs + chemicals = pattern warranting investigation" },
                ].map((item) => (
            ...
            <div className="rounded-2xl border p-5" style={{ background: "rgba(20,29,46,0.8)", borderColor: "rgba(181,142,60,0.12)", backdropFilter: "blur(12px)" }}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                {[
                  ...
                  { icon: "ri-money-dollar-circle-line", color: "#FB923C", label: isAr ? "معاملة عالية القيمة" : "High-Value Transaction", desc: isAr ? "شراء واحد فوق الحد المُهيَّأ حسب الفئة" : "Single purchase above configurable threshold by category" },
                  { icon: "ri-forbid-line", color: "#F87171", label: isAr ? "عنصر مقيّد" : "Restricted Item", desc: isAr ? "معدات مراقبة، مواد كيميائية، طائرات مسيّرة، أجهزة تشويش" : "Surveillance equipment, chemicals, drones, jammers" },
                  ...
                  { icon: "ri-exchange-line", color: "#D4A84B", label: isAr ? "نمط الدفع" : "Payment Pattern", desc: isAr ? "بطاقات متعددة، معاملات صغيرة متكررة، تجزئة" : "Multiple cards, frequent small transactions, structuring" },
            ...
            <div className="rounded-2xl border p-5" style={{ background: "rgba(20,29,46,0.8)", borderColor: "rgba(181,142,60,0.12)", backdropFilter: "blur(12px)" }}>
              <div className="flex items-center justify-between mb-5">
                ...
                <div className="flex items-center gap-3 flex-wrap">
                  {[{ label: "Bulk", color: "#FACC15" }, { label: "Restricted", color: "#F87171" }, { label: "High-Value", color: "#FB923C" }, { label: "Pattern", color: "#D4A84B" }].map((l) => (
                    ...
                    <div key={d.day} className="flex-1 flex flex-col items-center gap-1">
                      <div className="w-full flex flex-col-reverse gap-0.5" style={{ height: "100px" }}>
                        {[
                          ...
                          { val: d.restricted, color: "#F87171" },
                          { val: d.highvalue, color: "#FB923C" },
                          { val: d.pattern, color: "#D4A84B" },
                        ].map((seg, i) => (
            ...
            <div className="rounded-2xl border p-5" style={{ background: "rgba(20,29,46,0.8)", borderColor: "rgba(181,142,60,0.12)", backdropFilter: "blur(12px)" }}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                {[
                  { code: "AMN-ECM-20260405-0247", label: isAr ? "شراء مقيّد" : "Restricted Purchase", color: "#F87171" },
                  { code: "AMN-ECM-20260405-0246", label: isAr ? "شراء بالجملة" : "Bulk Purchase", color: "#FACC15" },
                  { code: "AMN-ECM-20260405-0245", label: isAr ? "معاملة عالية القيمة" : "High-Value Transaction", color: "#FB923C" },
                ].map((ex) => (
