    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          ...
          { label: isAr ? "متوسط الجودة" : "Avg Quality", value: `${Math.round(RETAILERS.filter((r) => r.status === "active").reduce((a, r) => a + r.dataQuality, 0) / RETAILERS.filter((r) => r.status === "active").length)}%`, color: "#22D3EE", icon: "ri-bar-chart-line" },
        ].map((s) => (
                      ...
                      <div className="flex gap-1 flex-wrap">
                        {r.categories.map((cat) => (
                          <span key={cat} className="px-1.5 py-0.5 rounded text-xs font-['JetBrains_Mono']"
                            style={{ background: "rgba(34,211,238,0.08)", color: "#22D3EE", fontSize: "9px" }}>{cat}</span>
                        ))}
                      </div>
                    </td>
                    <td className="px-4 py-3">
          ...
          {ONLINE_PLATFORMS.map((p) => {
            ...
            return (
              <div key={p.name} className="p-5">
                ...
                <div className="text-2xl font-black font-['JetBrains_Mono'] mb-1" style={{ color: "#22D3EE" }}>{p.eventsToday}</div>
