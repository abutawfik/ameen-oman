    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div key={stat.label} className="rounded-xl border p-4" style={{ background: "rgba(10,37,64,0.8)", borderColor: `${stat.color}20`, backdropFilter: "blur(12px)" }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
          ...
          {ANOMALIES.map((anomaly) => {
            ...
            return (
              <button key={anomaly.id} type="button" onClick={() => setSelectedId(isSelected ? null : anomaly.id)}
                ...
                style={{
                  background: isSelected ? `${color}08` : "rgba(10,37,64,0.8)",
                  borderColor: isSelected ? `${color}40` : "rgba(184,138,60,0.1)",
        ...
        <div className="lg:col-span-3">
          {selected ? (
            <div className="rounded-2xl border p-6 sticky top-24" style={{ background: "rgba(10,37,64,0.9)", borderColor: `${severityColor(selected.severity)}30`, backdropFilter: "blur(16px)" }}>
              {/* Header */}
            ...
            <div className="rounded-2xl border p-8 flex flex-col items-center justify-center text-center h-full min-h-[300px]" style={{ background: "rgba(10,37,64,0.5)", borderColor: "rgba(184,138,60,0.08)", backdropFilter: "blur(12px)" }}>
              <div className="w-14 h-14 flex items-center justify-center rounded-2xl mb-4" style={{ background: "rgba(184,138,60,0.08)", border: "1px solid rgba(184,138,60,0.15)" }}>
      ...
      <div className="rounded-2xl border p-5" style={{ background: "rgba(10,37,64,0.8)", borderColor: "rgba(184,138,60,0.12)", backdropFilter: "blur(12px)" }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {[
            { icon: "ri-bar-chart-grouped-line", label: isAr ? "ارتفاع مفاجئ >300% من المتوسط" : "Sudden spike >300% of average", color: "#C98A1B" },
            { icon: "ri-home-wifi-line", label: isAr ? "حساب نشط بدون إيجار مسجّل في البلدية" : "Active account, no rental in Municipality stream", color: "#C94A5E" },
