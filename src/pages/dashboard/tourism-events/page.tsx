...
import AdventureActivityForm from "./AdventureActivityForm";

type EventType = "attraction" | "tour" | "adventure";

const EVENT_CARDS = [
  { id: "attraction" as EventType, icon: "ri-ticket-2-line", label: "Attraction Entry", labelAr: "دخول معلم سياحي", desc: "Record visitor entry to tourist attractions, museums and cultural sites", descAr: "تسجيل دخول الزوار إلى المعالم السياحية والمتاحف والمواقع الثقافية", color: "#22D3EE", code: "TOR_ATTRACTION" },
      ...
      <main className="relative z-10 max-w-6xl mx-auto px-6 py-8">
        ...
        {activeEvent && (
          <>
            ...
            <div key={formKey}>
              {activeEvent === "attraction" && <AttractionEntryForm  isAr={isAr} onCancel={() => setActiveEvent(null)} />}
              {activeEvent === "tour"       && <TourBookingForm       isAr={isAr} onCancel={() => setActiveEvent(null)} />}
              {activeEvent === "adventure"  && <AdventureActivityForm isAr={isAr} onCancel={() => setActiveEvent(null)} />}
            </div>
          </>
        )}
      </main>
...
export default TourismEventsPage;
