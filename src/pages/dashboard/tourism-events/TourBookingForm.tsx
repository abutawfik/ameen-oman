      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
        <SectionCard title={isAr ? "تفاصيل الجولة" : "Tour Details"} icon="ri-map-2-line" accentColor="#4ADE80">
          <div className="space-y-4">
            ...
            {pricePerPerson && (
              <div className="flex items-center justify-between px-4 py-3 rounded-xl border" style={{ background: "rgba(34,211,238,0.04)", borderColor: "rgba(34,211,238,0.15)" }}>
                <span className="text-gray-400 text-xs">{isAr ? "الإجمالي للمجموعة" : "Group Total"}</span>
                <span className="text-cyan-400 font-bold font-['JetBrains_Mono']">LCY {isNaN(totalPrice) ? "0.000" : totalPrice.toFixed(3)}</span>
              </div>
            )}
          </div>
        </SectionCard>
