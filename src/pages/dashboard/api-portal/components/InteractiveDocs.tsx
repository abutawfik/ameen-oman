...
const methodColors: Record<string, { color: string; bg: string }> = {
  ...
  GET:    { color: "#D4A84B", bg: "rgba(181,142,60,0.12)" },
...
const InteractiveDocs = () => {
  ...
  return (
    <div className="flex gap-4 h-full" style={{ minHeight: "700px" }}>
      {/* Left: Endpoint Tree */}
      <div className="w-64 flex-shrink-0 rounded-xl overflow-hidden" style={{ background: "rgba(20,29,46,0.8)", border: "1px solid rgba(181,142,60,0.12)" }}>
        {/* Search */}
        <div className="p-3 border-b" style={{ borderColor: "rgba(181,142,60,0.08)" }}>
          <div className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(181,142,60,0.15)" }}>
        ...
        {/* Category filters */}
        <div className="p-2 border-b flex flex-wrap gap-1" style={{ borderColor: "rgba(181,142,60,0.08)" }}>
          {categories.map((c) => (
            <button key={c} onClick={() => setActiveCategory(c)}
              className="px-2 py-0.5 rounded text-xs cursor-pointer whitespace-nowrap"
              style={{ background: activeCategory === c ? "rgba(181,142,60,0.15)" : "transparent", color: activeCategory === c ? "#D4A84B" : "#6B7280" }}>
              {c}
            </button>
          ))}
        </div>
          ...
          {filteredEndpoints.map((ep) => {
            ...
            return (
              <button key={ep.id} onClick={() => handleSelectEndpoint(ep)}
                ...
                style={{ background: isActive ? "rgba(181,142,60,0.06)" : "transparent", borderLeft: isActive ? "2px solid #D4A84B" : "2px solid transparent" }}>
                ...
                <div className="min-w-0">
                  <p className="text-xs font-['JetBrains_Mono'] truncate" style={{ color: isActive ? "#D4A84B" : "#9CA3AF" }}>{ep.path}</p>
      ...
      <div className="flex-1 min-w-0 space-y-4">
        ...
        <div className="rounded-xl p-5" style={{ background: "rgba(20,29,46,0.8)", border: "1px solid rgba(181,142,60,0.12)" }}>
          <div className="flex items-center gap-3 mb-3">
            ...
            <span className="px-2 py-0.5 rounded text-xs font-['JetBrains_Mono']" style={{ background: "rgba(181,142,60,0.08)", color: "#D4A84B" }}>
        ...
        <div className="rounded-xl overflow-hidden" style={{ background: "rgba(20,29,46,0.8)", border: "1px solid rgba(181,142,60,0.12)" }}>
          <div className="flex border-b" style={{ borderColor: "rgba(181,142,60,0.08)" }}>
            {(["request", "response", "headers"] as const).map((tab) => (
              <button key={tab} onClick={() => setActiveTab(tab)}
                ...
                style={{ color: activeTab === tab ? "#D4A84B" : "#6B7280", borderBottom: activeTab === tab ? "2px solid #D4A84B" : "2px solid transparent", fontFamily: "'Inter', sans-serif" }}>
          ...
          <div className="p-4">
            {activeTab === "request" && (
              <pre className="text-xs font-['JetBrains_Mono'] overflow-auto" style={{ color: "#D4A84B", maxHeight: "200px" }}>
        ...
        {/* Try It Panel */}
        <div className="rounded-xl overflow-hidden" style={{ background: "rgba(20,29,46,0.8)", border: "1px solid rgba(181,142,60,0.2)" }}>
          <div className="flex items-center gap-3 px-5 py-3 border-b" style={{ borderColor: "rgba(181,142,60,0.1)", background: "rgba(181,142,60,0.04)" }}>
          ...
          <div className="p-4 space-y-3">
            ...
            <div>
              ...
              <input value={apiKeyInput} onChange={(e) => setApiKeyInput(e.target.value)}
                ...
                style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(181,142,60,0.2)", color: "#D4A84B" }} />
            </div>
            ...
            <button onClick={handleTryIt} disabled={tryItLoading}
              ...
              style={{ background: tryItLoading ? "rgba(181,142,60,0.4)" : "#D4A84B", color: "#0B1220" }}>
