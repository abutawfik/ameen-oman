...
const PstConfirmation = ({ refNumber, eventType, eventCode, color, isAr, onReset }: Props) => {
  ...
  return (
    <div className="rounded-2xl border p-10 flex flex-col items-center text-center" style={{ background: "rgba(10,22,40,0.9)", borderColor: `${color}30`, backdropFilter: "blur(20px)" }}>
      <div className="relative mb-6">
        <div className="w-24 h-24 flex items-center justify-center rounded-full" style={{ background: `${color}10`, border: `2px solid ${color}40`, boxShadow: `0 0 40px ${color}18` }}><i className="ri-checkbox-circle-line text-5xl" style={{ color }} /></div>
      ...
      <div className="px-8 py-5 rounded-2xl border mb-8 w-full max-w-md" style={{ background: "rgba(34,211,238,0.04)", borderColor: "rgba(34,211,238,0.15)" }}>
        <p className="text-gray-500 text-xs mb-2 uppercase tracking-widest">{isAr ? "رقم المرجع" : "Reference Number"}</p>
        <p className="text-xl font-bold font-['JetBrains_Mono'] tracking-wider mb-1" style={{ color: "#22D3EE" }}>{refNumber}</p>
        ...
        {steps.map((step, i) => (
          <div key={i} className="flex items-center gap-2">
            <div className="flex flex-col items-center gap-1">
              <div className="w-7 h-7 flex items-center justify-center rounded-full text-xs font-bold" style={{ background: i === 0 ? "rgba(34,211,238,0.2)" : "rgba(255,255,255,0.05)", border: `1px solid ${i === 0 ? "rgba(34,211,238,0.5)" : "rgba(255,255,255,0.1)"}`, color: i === 0 ? "#22D3EE" : "#6B7280" }}>{i === 0 ? <i className="ri-check-line text-xs" /> : i + 1}</div>
              <span className="text-xs whitespace-nowrap" style={{ color: i === 0 ? "#22D3EE" : "#6B7280" }}>{step}</span>
            </div>
            {i < steps.length - 1 && <div className="w-8 h-px mb-4" style={{ background: i === 0 ? "rgba(34,211,238,0.3)" : "rgba(255,255,255,0.08)" }} />}
          </div>
        ))}
      </div>
      <div className="flex gap-3">
        <button type="button" onClick={onReset} className="flex items-center gap-2 px-5 py-2.5 rounded-lg border text-sm font-semibold cursor-pointer whitespace-nowrap transition-colors" style={{ background: "transparent", borderColor: "rgba(34,211,238,0.25)", color: "#22D3EE" }} onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.background = "rgba(34,211,238,0.06)"; }} onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.background = "transparent"; }}><i className="ri-add-line" />{isAr ? "إضافة حدث جديد" : "Submit Another"}</button>
        <button type="button" onClick={() => navigate("/dashboard/postal-events")} className="flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-bold cursor-pointer whitespace-nowrap transition-colors" style={{ background: "#22D3EE", color: "#060D1A" }} onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.background = "#06B6D4"; }} onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.background = "#22D3EE"; }}><i className="ri-dashboard-line" />{isAr ? "العودة" : "Back to Events"}</button>
      </div>
    </div>
  );
};
export default PstConfirmation;
