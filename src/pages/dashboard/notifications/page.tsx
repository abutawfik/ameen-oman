...
const NotificationsPage = () => {
  ...
  const tabs: { key: MainTab; icon: string; labelEn: string; labelAr: string; badge?: number; badgeColor?: string }[] = [
    { key: "entity",        icon: "ri-notification-3-line",  labelEn: "Entity Notifications", labelAr: "إشعارات الكيان",    badge: unreadEntity, badgeColor: "#D6B47E" },
    { key: "police-alerts", icon: "ri-alarm-warning-line",   labelEn: "Police Alert Center",  labelAr: "مركز تنبيهات الشرطة",  badge: unackPolice,  badgeColor: criticalPolice > 0 ? "#C94A5E" : "#C98A1B" },
    ...
    <div className="flex flex-col h-full" style={{ background: "#051428" }}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative z-10">
        ...
        <div
          ...
          style={{ borderColor: "rgba(184,138,60,0.1)", background: "rgba(10,37,64,0.5)" }}
        >
          <div className="flex items-center gap-3">
            <div className="w-7 h-7 flex items-center justify-center rounded-lg" style={{ background: "rgba(184,138,60,0.1)", border: "1px solid rgba(184,138,60,0.3)" }}>
          ...
          <div className="flex items-center gap-3">
            {[
              { label: "Unread", labelAr: "غير مقروء", value: unreadEntity, color: "#D6B47E" },
              { label: "Unack'd", labelAr: "غير مؤكد", value: unackPolice, color: "#C98A1B" },
              { label: "Critical", labelAr: "حرج", value: criticalPolice, color: "#C94A5E" },
            ].map((stat) => (
        ...
        {/* Main tabs */}
        <div className="flex items-center gap-1 px-6 py-2 border-b flex-shrink-0" style={{ borderColor: "rgba(184,138,60,0.08)", background: "rgba(5,20,40,0.4)" }}>
          {tabs.map((tab) => (
            <button
              key={tab.key}
