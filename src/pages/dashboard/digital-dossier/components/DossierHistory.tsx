...
const statusConfig: Record<DossierStatus, { color: string; bg: string; label: string; icon: string }> = {
  ...
  generating: { color: "#22D3EE", bg: "rgba(34,211,238,0.1)",  label: "Generating", icon: "ri-loader-4-line" },
    ...
    <div className="flex gap-5 h-full">
      ...
      <div className="flex-1 space-y-4">
          ...
          <div className="flex gap-1.5">
            {(["all", "ready", "expired", "archived"] as const).map((s) => (
              <button
                ...
                style={{
                  background: statusFilter === s ? "rgba(34,211,238,0.1)" : "rgba(255,255,255,0.03)",
                  color: statusFilter === s ? "#22D3EE" : "#6B7280",
                  border: statusFilter === s ? "1px solid rgba(34,211,238,0.25)" : "1px solid rgba(255,255,255,0.06)",
                }}
              >
        ...
        <div className="grid grid-cols-4 gap-3">
          {[
            { label: "Total Generated", value: generatedDossiers.length, color: "#22D3EE" },
            ...
            <div key={stat.label} className="p-3 rounded-xl" style={{ background: "rgba(10,22,40,0.8)", border: "1px solid rgba(34,211,238,0.1)" }}>
          ...
          {filtered.map((dossier) => {
            ...
            return (
              <button
                ...
                style={{
                  background: isSelected ? "rgba(34,211,238,0.06)" : "rgba(10,22,40,0.8)",
                  border: isSelected ? "1px solid rgba(34,211,238,0.25)" : "1px solid rgba(34,211,238,0.1)",
                }}
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
                    ...
                    {dossier.status === "ready" && (
                      <div className="flex gap-1.5">
                        <button className="px-2.5 py-1 rounded text-[11px] font-['Inter'] cursor-pointer transition-colors whitespace-nowrap" style={{ background: "rgba(34,211,238,0.08)", color: "#22D3EE", border: "1px solid rgba(34,211,238,0.2)" }}>
                ...
                {/* Expanded audit log */}
                {isSelected && (
                  <div className="mt-4 pt-4 border-t" style={{ borderColor: "rgba(34,211,238,0.08)" }}>
                      ...
                      {dossier.auditLog.map((entry, idx) => (
                        <div key={idx} className="flex items-center gap-3 text-xs font-['JetBrains_Mono']">
                          ...
                          <span className="px-1.5 py-0.5 rounded text-[10px]" style={{
                            background: entry.action === "Generated" ? "rgba(34,211,238,0.1)" : entry.action === "Downloaded" ? "rgba(74,222,128,0.1)" : entry.action === "Expired" ? "rgba(251,146,60,0.1)" : "rgba(156,163,175,0.1)",
                            color: entry.action === "Generated" ? "#22D3EE" : entry.action === "Downloaded" ? "#4ADE80" : entry.action === "Expired" ? "#FB923C" : "#9CA3AF",
                          }}>
                            {entry.action}
                          </span>
