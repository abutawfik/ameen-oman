    ...
    <div className="flex flex-col h-full" style={{ background: "#0B1220" }}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative z-10">
        ...
        >
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg" style={{ background: "rgba(181,142,60,0.06)", border: "1px solid rgba(181,142,60,0.15)" }}>
        ...
        {/* KPI bar */}
        <div className="grid grid-cols-5 gap-0 border-b flex-shrink-0" style={{ borderColor: "rgba(181,142,60,0.08)" }}>
          {[
            { label: "Reports Generated", value: "1,247", icon: "ri-file-text-line", color: "#D4A84B" },
            ...
            <div
              ...
              style={{ borderRight: i < 4 ? "1px solid rgba(181,142,60,0.08)" : "none" }}
            >
        ...
        <div className="flex-1 overflow-hidden flex">
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            {/* Tabs */}
            <div className="flex items-center gap-1 px-6 py-3 border-b flex-shrink-0" style={{ borderColor: "rgba(181,142,60,0.08)" }}>
              {tabs.map((tab) => (
                <button
                  key={tab.key}
          ...
          <div className="w-72 flex-shrink-0 border-l flex flex-col overflow-hidden" style={{ borderColor: "rgba(181,142,60,0.1)" }}>
            {/* Generation status */}
            {generationState !== "idle" && (
              <div className="p-4 border-b" style={{ borderColor: "rgba(181,142,60,0.08)" }}>
                {generationState === "generating" && (
                  <div className="space-y-3">
                    <div className="flex items-center gap-2">
                      <div className="w-5 h-5 flex items-center justify-center">
                        <i className="ri-loader-4-line text-gold-400 text-sm animate-spin" />
                      </div>
                      <span className="text-gold-400 text-xs font-bold font-['JetBrains_Mono']">GENERATING...</span>
                    </div>
                    <div className="w-full h-1.5 rounded-full overflow-hidden" style={{ background: "rgba(181,142,60,0.1)" }}>
                      <div
                        className="h-full rounded-full transition-all duration-300"
                        style={{ width: `${generationProgress}%`, background: "linear-gradient(90deg, #D4A84B, #4ADE80)" }}
                      />
                    </div>
                    ...
                    <div className="space-y-1">
                      {generationSteps.slice(0, generationStep + 1).map((step, idx) => (
                        <div key={idx} className="flex items-center gap-1.5">
                          ...
                          <span className="text-[10px] font-['JetBrains_Mono']" style={{ color: idx < generationStep ? "#4ADE80" : "#D4A84B" }}>{step}</span>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            <div className="flex-1 overflow-y-auto p-4" style={{ scrollbarWidth: "thin", scrollbarColor: "rgba(181,142,60,0.2) transparent" }}>
              ...
              <div className="space-y-1.5">
                {[
                  { stream: "Identity Fusion",   icon: "ri-fingerprint-line",      color: "#D4A84B", events: 12, status: "active" },
                  { stream: "Border Control",    icon: "ri-passport-line",          color: "#60A5FA", events: 3,  status: "active" },
                  { stream: "Hotel & Hospitality",icon: "ri-hotel-line",            color: "#D4A84B", events: 3,  status: "active" },
                  ...
                  { stream: "Marine & Maritime", icon: "ri-anchor-line",            color: "#DDB96B", events: 1,  status: "active" },
