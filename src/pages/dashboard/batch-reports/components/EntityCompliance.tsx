...
const ENTITIES = [
  ...
  { name: "Mwasalat",                  nameAr: "مواصلات",                  icon: "ri-bus-line",           color: "#C98A1B", rating: "silver", score: 91, events: 42891,rejected: 156,trend: [82,84,85,86,87,88,89,90,90,91,91,91] },
    ...
    <div className="space-y-6">
              ...
              {ENTITIES.map((entity, idx) => {
                ...
                return (
                  <tr key={entity.name}
                    ...
                    style={{
                      background: selected?.name === entity.name ? "rgba(184,138,60,0.05)" : idx % 2 === 0 ? "rgba(10,37,64,0.6)" : "rgba(5,20,40,0.4)",
                      borderColor: "rgba(184,138,60,0.05)",
                    }}
                    onMouseEnter={(e) => { if (selected?.name !== entity.name) (e.currentTarget as HTMLElement).style.background = "rgba(184,138,60,0.03)"; }}
                    onMouseLeave={(e) => { if (selected?.name !== entity.name) (e.currentTarget as HTMLElement).style.background = idx % 2 === 0 ? "rgba(10,37,64,0.6)" : "rgba(5,20,40,0.4)"; }}>
                    ...
                    <td className="px-4 py-3">
                      <span className="text-xs font-['JetBrains_Mono']"
                        style={{ color: Number(rejectRate) < 1 ? "#4ADE80" : Number(rejectRate) < 3 ? "#FACC15" : "#C94A5E" }}>
                        {rejectRate}%
                      </span>
                    </td>
                    <td className="px-4 py-3">
      ...
      {selected && (
        <div className="rounded-2xl border p-6"
          style={{ background: "rgba(5,20,40,0.95)", borderColor: "rgba(184,138,60,0.2)", backdropFilter: "blur(16px)" }}>
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              ...
              { label: isAr ? "المرفوضة" : "Rejected", value: selected.rejected.toString(), color: "#C94A5E" },
