  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 rounded-2xl border p-5"
        ...
        <div className="flex flex-wrap items-center gap-3">
          <select value={entityType} onChange={(e) => setEntityType(e.target.value)}
            ...
            style={{ background: "rgba(6,13,26,0.8)", borderColor: "rgba(34,211,238,0.2)", color: "#D1D5DB", minWidth: "180px" }}>
          ...
          <button type="button"
            ...
            style={{ background: "transparent", border: "1px solid #22D3EE", color: "#22D3EE" }}>
      ...
      <div
        ...
        style={{
          borderColor: isDragging ? "#22D3EE" : "rgba(34,211,238,0.25)",
          background: isDragging ? "rgba(34,211,238,0.06)" : "rgba(10,22,40,0.6)",
        ...
        <div className="w-16 h-16 flex items-center justify-center rounded-2xl mb-4"
          style={{ background: isDragging ? "rgba(34,211,238,0.15)" : "rgba(34,211,238,0.08)", border: `1px solid ${isDragging ? "#22D3EE" : "rgba(34,211,238,0.2)"}` }}>
          <i className={`ri-upload-cloud-2-line text-3xl ${isDragging ? "text-cyan-300" : "text-cyan-400"}`} />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {files.map((file) => (
            <div key={file.id} className="rounded-2xl border p-4 flex items-start gap-4"
              style={{ background: "rgba(10,22,40,0.8)", borderColor: file.done ? "rgba(74,222,128,0.2)" : "rgba(34,211,238,0.12)", backdropFilter: "blur(12px)" }}>
              <div className="w-10 h-10 flex items-center justify-center rounded-xl flex-shrink-0"
                style={{ background: file.done ? "rgba(74,222,128,0.1)" : "rgba(34,211,238,0.1)", border: `1px solid ${file.done ? "rgba(74,222,128,0.3)" : "rgba(34,211,238,0.2)"}` }}>
                <i className={`${file.name.endsWith(".csv") ? "ri-file-text-line" : "ri-file-excel-2-line"} text-base`}
                  style={{ color: file.done ? "#4ADE80" : "#22D3EE" }} />
              </div>
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-xs font-['JetBrains_Mono']" style={{ color: file.done ? "#4ADE80" : "#22D3EE" }}>
                ...
                <div className="h-1.5 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.06)" }}>
                  <div className="h-full rounded-full transition-all duration-300"
                    style={{ width: `${file.progress}%`, background: file.done ? "#4ADE80" : "linear-gradient(90deg, #22D3EE, #38BDF8)" }} />
                </div>
      ...
      {processing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(6,13,26,0.85)", backdropFilter: "blur(8px)" }}>
          <div className="rounded-2xl border p-8 w-full max-w-md mx-4 text-center"
            style={{ background: "rgba(10,22,40,0.95)", borderColor: "rgba(34,211,238,0.25)", backdropFilter: "blur(20px)" }}>
            {/* Spinner */}
            <div className="w-16 h-16 mx-auto mb-5 relative">
              ...
              <div className="absolute inset-2 rounded-full flex items-center justify-center"
                ...
                <i className="ri-upload-cloud-2-line text-cyan-400 text-xl" />
              </div>
            </div>
            ...
            <div className="flex items-center justify-center gap-2 mb-4">
              <span className="text-cyan-400 font-bold font-['JetBrains_Mono'] text-2xl">{files.length}</span>
            ...
            <div className="flex items-center justify-center gap-2">
              {PROCESSING_STEPS.map((_, i) => (
                <div key={i} className="w-2 h-2 rounded-full transition-all"
                  style={{ background: i <= processingStep ? "#22D3EE" : "rgba(255,255,255,0.1)" }} />
              ))}
            </div>
          </div>
        </div>
