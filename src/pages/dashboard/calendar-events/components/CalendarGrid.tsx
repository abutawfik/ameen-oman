...
const ENTITY_LEGENDS: Record<string, { label: string; labelAr: string; color: string; icon: string }[]> = {
  ...
  transport: [
    ...
    { label: "Anomaly",        labelAr: "شذوذ",         color: "#C94A5E", icon: "ri-alert-line" },
...
const statusColor = (s: string) => s === "accepted" ? "#4ADE80" : s === "pending" ? "#FACC15" : "#C94A5E";
...
const CalendarGrid = ({ isAr, entityType }: Props) => {
  ...
  return (
    <div className="rounded-2xl border overflow-hidden" style={{ background: "rgba(10,37,64,0.8)", borderColor: "rgba(184,138,60,0.12)", backdropFilter: "blur(12px)" }}>
      {/* Calendar header */}
      <div className="flex items-center justify-between px-6 py-4 border-b" style={{ borderColor: "rgba(184,138,60,0.08)" }}>
      ...
      {/* Legend */}
      <div className="flex flex-wrap items-center gap-x-5 gap-y-2 px-6 py-3 border-b" style={{ borderColor: "rgba(184,138,60,0.06)", background: "rgba(0,0,0,0.15)" }}>
        {legend.map((l) => (
        ...
        <div className="ml-auto flex items-center gap-4">
          ...
          <div className="flex items-center gap-1.5">
            <div className="w-8 h-1.5 rounded-full flex-shrink-0" style={{ background: "rgba(184,138,60,0.4)" }} />
      ...
      {/* Day headers */}
      <div className="grid grid-cols-7 border-b" style={{ borderColor: "rgba(184,138,60,0.06)" }}>
      ...
      <div className="grid grid-cols-7">
        {Array.from({ length: firstDay }).map((_, i) => (
          <div key={`empty-${i}`} className="min-h-[100px] border-b border-r" style={{ borderColor: "rgba(184,138,60,0.04)", background: "rgba(0,0,0,0.15)" }} />
        ))}
        ...
        {Array.from({ length: daysInMonth }).map((_, i) => {
          ...
          return (
            <div key={day}
              ...
              style={{
                borderColor: "rgba(184,138,60,0.06)",
                background: isSelected
                  ? "rgba(184,138,60,0.08)"
                  : isToday
                  ? "rgba(184,138,60,0.03)"
                  : "rgba(10,37,64,0.4)",
                outline: isSelected ? "1px solid rgba(184,138,60,0.3)" : "none",
              }}
              onMouseEnter={(e) => { if (!isSelected) (e.currentTarget as HTMLElement).style.background = "rgba(184,138,60,0.04)"; }}
              onMouseLeave={(e) => { if (!isSelected) (e.currentTarget as HTMLElement).style.background = isToday ? "rgba(184,138,60,0.03)" : "rgba(10,37,64,0.4)"; }}>
      ...
      {/* Day detail popup */}
      {selectedDay && (
        <div className="border-t" style={{ borderColor: "rgba(184,138,60,0.15)" }}>
          <div className="p-6">
            {/* Popup header */}
            ...
            {selectedEvents.length > 0 ? (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 mb-4">
                {selectedEvents.map((ev) => (
                  <div key={ev.id} className="flex items-start gap-3 p-3 rounded-xl border transition-colors cursor-pointer"
                    style={{ background: "rgba(5,20,40,0.6)", borderColor: `${ev.color}20` }}
              ...
              <div className="flex flex-col items-center justify-center py-8 mb-4 rounded-xl border"
                style={{ background: "rgba(5,20,40,0.4)", borderColor: "rgba(184,138,60,0.08)" }}>
