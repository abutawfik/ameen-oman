...
const ALL_EVENTS: EventRow[] = [
  ...
  { id: "r2",  type: "Booking Created",       typeAr: "إنشاء حجز",          typeColor: "#22D3EE", ref: "HTL-2025-04890", status: "accepted", person: "Sarah Johnson",       docMasked: "***8823", date: "2026-04-05", time: "10:30", amount: "OMR 890.000",    branch: "Al Bustan Palace",    branchAr: "البستان بالاس",    detail: "Suite 512, 3 nights",             detailAr: "جناح 512، 3 ليالٍ",             nationality: "GB", phone: "+44 7700 900123", module: "hotel" },
...
const EventListTable = ({ isAr, entityType }: Props) => {
  ...
  const SortIcon = ({ col }: { col: string }) => (
    <i className={`ml-1 text-xs ${sortCol === col ? (sortDir === "asc" ? "ri-arrow-up-line text-cyan-400" : "ri-arrow-down-line text-cyan-400") : "ri-arrow-up-down-line text-gray-600"}`} />
  );
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border p-5" style={{ background: "rgba(10,22,40,0.8)", borderColor: "rgba(34,211,238,0.12)", backdropFilter: "blur(12px)" }}>
        <div className="flex flex-wrap items-end gap-3">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <select value={filterType} onChange={(e) => { setFilterType(e.target.value); setPage(1); }}
              ...
              style={{ background: "rgba(6,13,26,0.8)", borderColor: "rgba(34,211,238,0.15)", color: "#D1D5DB", minWidth: "160px" }}>
        ...
        {selectedBulk.length > 0 && (
          <div className="mt-3 flex items-center gap-3 px-4 py-2.5 rounded-xl border"
            ...
            <i className="ri-checkbox-multiple-line text-cyan-400 text-sm" />
            <span className="text-cyan-400 text-xs font-semibold font-['JetBrains_Mono']">
      ...
      <div className="flex items-center justify-between px-1">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-cyan-400 text-xs font-['JetBrains_Mono']">{sortCol} {sortDir === "asc" ? "↑" : "↓"}</span>
        </div>
      </div>
      ...
      <div className="rounded-2xl border overflow-hidden" style={{ background: "rgba(10,22,40,0.8)", borderColor: "rgba(34,211,238,0.12)", backdropFilter: "blur(12px)" }}>
            ...
            <thead>
              <tr style={{ background: "rgba(34,211,238,0.05)", borderBottom: "1px solid rgba(34,211,238,0.1)" }}>
                <th className="w-10 px-4 py-3">
                  <input type="checkbox" className="cursor-pointer accent-cyan-400"
                ...
                {[
                  ...
                  <th key={col.key}
                    ...
                    style={{ color: sortCol === col.key ? "#22D3EE" : "#6B7280" }}>
                ...
                <tr key={row.id}
                  ...
                  style={{
                    background: selectedRow?.id === row.id
                      ...
                      : idx % 2 === 0 ? "rgba(10,22,40,0.6)" : "rgba(6,13,26,0.4)",
                    borderColor: "rgba(34,211,238,0.05)",
                    borderLeft: selectedRow?.id === row.id ? "2px solid #22D3EE" : "2px solid transparent",
                  }}
                  onMouseEnter={(e) => { if (selectedRow?.id !== row.id) (e.currentTarget as HTMLElement).style.background = "rgba(34,211,238,0.04)"; }}
                  ...
                  <td className="px-4 py-3">
                    <span className="text-cyan-400 text-xs font-['JetBrains_Mono'] cursor-pointer hover:underline whitespace-nowrap">{row.ref}</span>
                  </td>
                  <td className="px-4 py-3">
        ...
        {/* Pagination */}
        <div className="flex items-center justify-between px-6 py-4 border-t" style={{ borderColor: "rgba(34,211,238,0.08)" }}>
            ...
            {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
              ...
              return (
                <button key={p} type="button" onClick={() => setPage(p)}
                  ...
                  style={{
                    background: page === p ? "#22D3EE" : "transparent",
                    borderColor: page === p ? "#22D3EE" : "rgba(34,211,238,0.2)",
                    color: page === p ? "#060D1A" : "#22D3EE",
                  }}>
                  {p}
                </button>
              );
      ...
      {selectedRow && (
        <div className="rounded-2xl border overflow-hidden"
          style={{ background: "rgba(6,13,26,0.97)", borderColor: "rgba(34,211,238,0.25)", backdropFilter: "blur(20px)" }}>
          {/* Panel header */}
          <div className="flex items-center justify-between px-6 py-4 border-b" style={{ borderColor: "rgba(34,211,238,0.1)", background: "rgba(34,211,238,0.04)" }}>
          ...
          <div className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mb-6">
              {[
                { icon: "ri-user-line",        color: "#22D3EE", label: isAr ? "الشخص" : "Person",       value: selectedRow.person },
                ...
                { icon: "ri-calendar-line",    color: "#22D3EE", label: isAr ? "التاريخ والوقت" : "Date & Time", value: `${selectedRow.date} — ${selectedRow.time}` },
            ...
            <div className="flex items-center gap-3 px-4 py-3 rounded-xl mb-5"
              ...
              <div>
                ...
                <div className="text-cyan-400 text-sm font-bold font-['JetBrains_Mono']">AMN-EVT-{selectedRow.date.replace(/-/g, "")}-{selectedRow.ref.split("-").pop()}</div>
              </div>
            </div>
