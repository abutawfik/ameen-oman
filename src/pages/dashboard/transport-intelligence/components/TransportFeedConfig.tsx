...
const PROVIDERS = [
  { id: "nat-bus", name: "National Bus Co.", nameAr: "شركة الحافلات الوطنية", type: "Government Bus", typeAr: "حافلة حكومية", icon: "ri-bus-line", color: "#D4A84B", status: "online" as const, tripsToday: 14821, matchRate: 94, lastSync: "14:32:07" },
...
const MATCH_METHODS = [
  { id: "transit-card", icon: "ri-bank-card-line", label: "Transit Card → Document", labelAr: "بطاقة عبور → وثيقة", enabled: true, color: "#D4A84B", matched: 8241 },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: isAr ? "إجمالي الرحلات اليوم" : "Total Trips Today", value: "27,002", color: "#D4A84B", icon: "ri-route-line" },
          ...
          {PROVIDERS.map((p) => (
            <div key={p.id} className="flex items-center gap-4 px-6 py-4 hover:bg-white/[0.02] transition-colors">
              ...
              <div className="hidden lg:block text-right">
                <div className="text-gold-400 text-xs font-['JetBrains_Mono']">{p.lastSync}</div>
          ...
          {methods.map((m) => (
            <div key={m.id} className="flex items-center gap-4 px-6 py-4">
              ...
              <button type="button" onClick={() => toggleMethod(m.id)}
                ...
                style={{ background: m.enabled ? "#D4A84B" : "rgba(255,255,255,0.1)" }}>
