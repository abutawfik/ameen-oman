...
const ENTITIES_DATA = [
  ...
  { id: "ENT-009", name: "Mwasalat",                  nameAr: "مواصلات",                 type: "Transport",    typeIcon: "ri-bus-line",           color: "#FB923C", status: "active",    integration: "API",    events: 42891,lastActivity: "14:30:40" },
...
const CommandCenterPage = () => {
  ...
  const TABS: { id: Tab; icon: string; label: string; labelAr: string; badge?: number; badgeColor?: string }[] = [
    ...
    { id: "alerts",      icon: "ri-alarm-warning-line",  label: "Risk Alerts",       labelAr: "تنبيهات المخاطر", badge: openAlerts, badgeColor: "#F87171" },
                ...
                {filteredFeed.map((item) => (
                  <div key={item.id} className="flex items-center gap-3 px-5 py-2.5 hover:bg-white/[0.02] transition-colors cursor-pointer">
                    ...
                    <div className="flex items-center gap-1.5 flex-shrink-0">
                      {item.risk === "flagged" && (
                        <div className="flex items-center gap-1 px-2 py-0.5 rounded-full" style={{ background: "rgba(248,113,113,0.12)", border: "1px solid rgba(248,113,113,0.3)" }}>
            ...
            <div className="rounded-2xl border overflow-hidden"
              ...
              <div className="p-5">
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-5">
                  {[
                    { label: isAr ? "ضبط اليوم" : "Seizures Today",      value: "23",       color: "#F87171", icon: "ri-shield-cross-line",        sub: "+21% vs yesterday" },
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {[
                      { code: "9301", desc: "Military weapons & ammunition",       risk: "critical", score: 98, seizures: 3,  color: "#F87171" },
                      { code: "2933", desc: "Heterocyclic compounds (precursors)", risk: "critical", score: 94, seizures: 5,  color: "#F87171" },
                      { code: "8471", desc: "Computing machines & components",     risk: "high",     score: 78, seizures: 8,  color: "#FB923C" },
                      { code: "6110", desc: "Jerseys & pullovers (textile)",       risk: "high",     score: 72, seizures: 14, color: "#FB923C" },
          ...
          <>
            ...
            <div className="grid grid-cols-3 gap-4">
              {[
                ...
                { label: isAr ? "موقوف" : "Suspended",value: ENTITIES_DATA.filter((e) => e.status === "suspended").length, color: "#F87171" },
                    ...
                    <div
                      ...
                      <div className="flex items-center gap-4 px-5 py-3 flex-wrap">
                        ...
                        <div className="flex items-center gap-1 flex-shrink-0">
                          <span className="text-xs font-bold font-['JetBrains_Mono']" style={{ color: alert.score >= 80 ? "#F87171" : alert.score >= 60 ? "#FACC15" : "#4ADE80" }}>
                            {alert.score}
                          </span>
                        ...
                        <span className="px-2 py-0.5 rounded-full text-xs font-semibold flex-shrink-0"
                          style={{
                            background: alert.status === "resolved" ? "rgba(74,222,128,0.1)" : alert.status === "escalated" ? "rgba(248,113,113,0.1)" : alert.status === "investigating" ? "rgba(250,204,21,0.1)" : "rgba(156,163,175,0.1)",
                            color: alert.status === "resolved" ? "#4ADE80" : alert.status === "escalated" ? "#F87171" : alert.status === "investigating" ? "#FACC15" : "#9CA3AF",
                          }}>
                        ...
                        {alert.minutesOpen > 0 && alert.status !== "resolved" && (
                          <span className="text-xs font-['JetBrains_Mono'] flex-shrink-0" style={{ color: alert.minutesOpen >= 30 ? "#F87171" : "#FACC15" }}>
                            {alert.minutesOpen}m
                          </span>
                        )}
                      </div>
                          ...
                          {alert.status !== "resolved" && (
                            <>
                              ...
                              <button type="button" onClick={() => escalateAlert(alert.id)}
                                ...
                                style={{ background: "transparent", borderColor: "rgba(248,113,113,0.4)", color: "#F87171" }}>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            <div className="w-2 h-2 rounded-full animate-pulse" style={{ background: lagOk ? "#4ADE80" : "#F87171" }} />
                            <span className="text-xs font-semibold" style={{ color: lagOk ? "#4ADE80" : "#F87171" }}>
                    ...
                    <div className="p-5 space-y-4">
                      ...
                      {[
                        ...
                        <div key={s.label} className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-1.5">
                            <div className="w-1.5 h-1.5 rounded-full" style={{ background: s.ok ? "#4ADE80" : "#F87171" }} />
                            <span className="text-xs font-['JetBrains_Mono']" style={{ color: s.ok ? "#4ADE80" : "#F87171" }}>{s.value}</span>
                          </div>
                        </div>
                      ))}
                      {!lagOk && (
