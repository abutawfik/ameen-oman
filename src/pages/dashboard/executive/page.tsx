...
const ExecutiveDashboardPage = () => {
  ...
  const priorityColors = { critical: "#C94A5E", high: "#C98A1B", medium: "#FACC15" };
  const threatLevelColors = { critical: "#C94A5E", high: "#C98A1B", medium: "#FACC15", low: "#4ADE80" };
  ...
  return (
    <div className="flex flex-col h-full" style={{ background: "#051428" }}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative z-10">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-3 border-b flex-shrink-0" style={{ borderColor: "rgba(184,138,60,0.1)", background: "rgba(10,37,64,0.7)" }}>
        ...
        {/* Scrollable content */}
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-5" style={{ scrollbarWidth: "thin", scrollbarColor: "rgba(184,138,60,0.2) transparent" }}>
              ...
              {streamHealth.map((s, i) => (
                <div key={s.stream} className="flex items-center gap-3 px-4 py-3" style={{ borderRight: (i + 1) % 4 !== 0 ? "1px solid rgba(255,255,255,0.04)" : "none", borderBottom: i < 12 ? "1px solid rgba(255,255,255,0.04)" : "none" }}>
                  ...
                  <div className="flex flex-col items-end gap-1 flex-shrink-0">
                    ...
                    {s.alertsToday > 0 && (
                      <span className="text-[9px] font-['JetBrains_Mono']" style={{ color: "#C94A5E" }}>{s.alertsToday}⚠</span>
                    )}
                  </div>
                </div>
              ))}
