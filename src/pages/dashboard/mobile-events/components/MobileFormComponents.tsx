      ...
      <div className="relative">
        <input
          ...
          style={{
            ...
            boxShadow: isError ? "0 0 0 2px rgba(201,74,94,0.06)" : isValid ? "0 0 0 2px rgba(74,222,128,0.06)" : "none",
          }}
          onFocus={(e) => {
            if (!isError && !isValid) {
...
export const ServiceToggle = ({ label, labelAr, icon, value, onChange, isAr }: ServiceToggleProps) => (
  <button
    ...
    style={{
      background: value ? "rgba(184,138,60,0.08)" : "rgba(255,255,255,0.02)",
      borderColor: value ? "rgba(184,138,60,0.35)" : "rgba(255,255,255,0.08)",
    }}
  >
    <div
      className="w-8 h-8 flex items-center justify-center rounded-lg flex-shrink-0 transition-all"
      style={{
        background: value ? "rgba(184,138,60,0.15)" : "rgba(255,255,255,0.04)",
...
export const CountryChipSelector = ({ selected, onChange, isAr }: CountryChipProps) => {
  ...
  return (
    <div>
      ...
      {/* Selected chips */}
      {selected.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mb-2 p-2 rounded-lg" style={{ background: "rgba(184,138,60,0.04)", border: "1px solid rgba(184,138,60,0.12)" }}>
          {selected.map((c) => (
            <span
              key={c}
...
export const MobileConfirmationPanel = ({ refNumber, eventType, isAr, onReset, onDashboard }: ConfirmProps) => (
  ...
  >
    <div
      ...
      style={{ background: "rgba(184,138,60,0.1)", border: "2px solid rgba(184,138,60,0.4)", boxShadow: "0 0 40px rgba(184,138,60,0.15)" }}
    >
