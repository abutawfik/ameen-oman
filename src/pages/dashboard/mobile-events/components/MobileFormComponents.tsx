...
export const ServiceToggle = ({ label, labelAr, icon, value, onChange, isAr }: ServiceToggleProps) => (
  <button
    ...
    style={{
      background: value ? "rgba(34,211,238,0.08)" : "rgba(255,255,255,0.02)",
      borderColor: value ? "rgba(34,211,238,0.35)" : "rgba(255,255,255,0.08)",
    }}
  >
    <div
      className="w-8 h-8 flex items-center justify-center rounded-lg flex-shrink-0 transition-all"
      style={{
        background: value ? "rgba(34,211,238,0.15)" : "rgba(255,255,255,0.04)",
        border: `1px solid ${value ? "rgba(34,211,238,0.3)" : "rgba(255,255,255,0.08)"}`,
      }}
    >
      <i className={`${icon} text-sm`} style={{ color: value ? "#22D3EE" : "#6B7280" }} />
    </div>
    <span className="text-sm font-semibold font-['Inter'] flex-1" style={{ color: value ? "#22D3EE" : "#9CA3AF" }}>
      {isAr ? labelAr : label}
    </span>
    <div
...
export const CountryChipSelector = ({ selected, onChange, isAr }: CountryChipProps) => {
  ...
  return (
    <div>
      ...
      {/* Selected chips */}
      {selected.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mb-2 p-2 rounded-lg" style={{ background: "rgba(34,211,238,0.04)", border: "1px solid rgba(34,211,238,0.12)" }}>
          {selected.map((c) => (
            <span
              key={c}
      ...
      {selected.length > 0 && (
        <p className="text-cyan-400 text-xs mt-1.5 font-['Inter']">
...
export const MobileConfirmationPanel = ({ refNumber, eventType, isAr, onReset, onDashboard }: ConfirmProps) => (
  ...
  >
    <div
      ...
      style={{ background: "rgba(34,211,238,0.1)", border: "2px solid rgba(34,211,238,0.4)", boxShadow: "0 0 40px rgba(34,211,238,0.15)" }}
    >
...
export const LookupBar = ({ label, labelAr, placeholder, value, onChange, onLookup, isAr, extra }: LookupBarProps) => (
  ...
  >
    <div className="flex items-center gap-2 mb-4 pb-3 border-b" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
      <div className="w-7 h-7 flex items-center justify-center rounded-lg" style={{ background: "rgba(34,211,238,0.1)", border: "1px solid rgba(34,211,238,0.2)" }}>
        <i className="ri-search-line text-cyan-400 text-sm" />
      </div>
    ...
    <div className="flex gap-3 items-end flex-wrap">
      <div className="flex-1 min-w-48">
        <label className="block text-gray-400 text-xs mb-1.5 font-['Inter']">
          {isAr ? labelAr : label}<span className="text-cyan-400 ml-0.5">*</span>
        </label>
        <div className="flex gap-2">
          ...
          <button
            ...
            onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.background = "rgba(34,211,238,0.2)"; }}
