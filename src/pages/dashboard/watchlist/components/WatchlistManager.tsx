...
const typeConfig: Record<WatchlistType, { label: string; labelAr: string; color: string; icon: string }> = {
  ...
  custom:            { label: 'Custom', labelAr: 'مخصص', color: '#22D3EE', icon: 'ri-settings-3-line' },
};
    ...
    <div className="flex gap-4 h-full">
        ...
        {watchlists.map(wl => {
          ...
          return (
            <button key={wl.id} onClick={() => setActiveWlId(wl.id)}
              ...
              style={{
                background: isActive ? 'rgba(34,211,238,0.1)' : 'rgba(10,22,40,0.6)',
                border: isActive ? `1px solid ${cfg.color}60` : '1px solid rgba(34,211,238,0.08)',
      ...
      <div className="flex-1 space-y-4 min-w-0">
        ...
        <div className="rounded-xl p-4"
          style={{ background: 'rgba(10,22,40,0.8)', border: `1px solid ${typeConfig[activeWl.type].color}30` }}>
          ...
          <div className="grid grid-cols-4 gap-4 mt-4 pt-4 border-t" style={{ borderColor: 'rgba(34,211,238,0.08)' }}>
            {[
              ...
              { label: 'Auto-Expire', labelAr: 'انتهاء تلقائي', value: activeWl.autoExpireDays ? `${activeWl.autoExpireDays}d` : 'Permanent', color: '#22D3EE' },
              ...
              <tbody>
                {filteredTargets.map((t, i) => (
                  <tr key={t.id}
                    ...
                    style={{ borderBottom: i < filteredTargets.length - 1 ? '1px solid rgba(34,211,238,0.05)' : 'none' }}>
                    ...
                    <td className="px-4 py-3">
                      <p className="text-cyan-400 text-xs font-['JetBrains_Mono']">{t.docNumber}</p>
            ...
            <div className="space-y-3">
              <div className="relative">
                ...
                <input placeholder={isAr ? 'بحث بالوثيقة أو الاسم أو الهاتف...' : 'Search by document, name, or phone...'}
                  ...
                  style={{ background: 'rgba(6,13,26,0.8)', border: '1px solid rgba(34,211,238,0.2)', color: '#D1D5DB' }} />
              </div>
              ...
              <div className="rounded-xl overflow-hidden" style={{ border: '1px solid rgba(34,211,238,0.1)' }}>
                {watchlistTargets.slice(0, 3).map((t, i) => (
                  <div key={t.id} className="flex items-center gap-3 px-3 py-2.5 cursor-pointer hover:bg-white/5 transition-colors"
                    style={{ borderBottom: i < 2 ? '1px solid rgba(34,211,238,0.05)' : 'none' }}>
              ...
              <div className="flex items-center gap-2 p-3 rounded-xl"
                ...
                <i className="ri-upload-2-line text-cyan-400 text-sm" />
