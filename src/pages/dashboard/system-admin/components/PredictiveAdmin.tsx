...
const severityConfig: Record<string, { color: string; bg: string }> = {
  ...
  low:      { color: "#22D3EE", bg: "rgba(34,211,238,0.12)" },
};
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-5 gap-3">
        {[
          { label: "Active Rules", value: String(activeCount), sub: `of ${rules.length} total`, color: "#22D3EE", icon: "ri-git-branch-line" },
          ...
          { label: "F1 Score", value: `${f1Score}%`, sub: "harmonic mean", color: "#22D3EE", icon: "ri-bar-chart-grouped-line" },
          { label: "Avg False Positive", value: `${avgFPR}%`, sub: "false positive rate", color: "#FACC15", icon: "ri-error-warning-line" },
        ].map((s) => (
          <div key={s.label} className="rounded-xl p-4" style={{ background: "rgba(10,22,40,0.8)", border: "1px solid rgba(34,211,238,0.12)" }}>
      ...
      <div className="rounded-xl p-5" style={{ background: "rgba(10,22,40,0.8)", border: "1px solid rgba(34,211,238,0.12)" }}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-white font-semibold text-sm font-['Inter']">
            <i className="ri-bar-chart-horizontal-line mr-2 text-cyan-400" />Rule Performance Metrics
          </h3>
          <div className="flex gap-1.5">
            {[
              { id: "precision", label: "Precision", color: "#22D3EE" },
        ...
        <div className="space-y-2">
          {metricData[activeMetricTab].map((rule, i) => {
            ...
            const color = activeMetricTab === "fpr"
              ...
              : activeMetricTab === "precision" ? "#22D3EE" : "#A78BFA";
      ...
      <div>
        ...
        <div className="flex items-center gap-3 mb-3 flex-wrap">
          <div className="flex gap-1.5 flex-wrap">
            {categories.map((c) => (
              <button key={c} onClick={() => setFilterCategory(c)}
                ...
                style={{ background: filterCategory === c ? "#22D3EE" : "rgba(255,255,255,0.04)", color: filterCategory === c ? "#060D1A" : "#9CA3AF", border: filterCategory === c ? "none" : "1px solid rgba(255,255,255,0.06)" }}>
            ...
            {severities.map((s) => {
              ...
              return (
                <button key={s} onClick={() => setFilterSeverity(s)}
                  className="px-3 py-1.5 rounded-full text-xs font-semibold cursor-pointer whitespace-nowrap capitalize"
                  style={{ background: filterSeverity === s ? (sc?.bg || "rgba(34,211,238,0.12)") : "rgba(255,255,255,0.04)", color: filterSeverity === s ? (sc?.color || "#22D3EE") : "#9CA3AF", border: filterSeverity === s ? "none" : "1px solid rgba(255,255,255,0.06)" }}>
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <select value={sortBy} onChange={(e) => setSortBy(e.target.value as typeof sortBy)}
              ...
              <option value="triggeredTotal" style={{ background: "#0A1628" }}>Total Triggers</option>
              <option value="triggeredToday" style={{ background: "#0A1628" }}>Today&apos;s Triggers</option>
              <option value="falsePositiveRate" style={{ background: "#0A1628" }}>False Positive Rate</option>
              <option value="precision" style={{ background: "#0A1628" }}>Precision</option>
            </select>
          </div>
        </div>
        ...
        <div className="rounded-xl overflow-hidden" style={{ border: "1px solid rgba(34,211,238,0.12)" }}>
          <div className="grid px-4 py-3 text-xs font-semibold uppercase tracking-wider font-['Inter'] text-gray-600"
            style={{ background: "rgba(34,211,238,0.04)", gridTemplateColumns: "2fr 1fr 1fr 1fr 1fr 1fr 1fr 70px" }}>
            ...
            return (
              <div key={rule.id}>
                ...
                >
                  ...
                  <span className="text-cyan-400 text-xs font-['JetBrains_Mono']">{rule.triggeredToday}</span>
                  ...
                  <div className="flex items-center gap-1.5">
                    <div className="w-12 h-1.5 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.06)" }}>
                      <div className="h-full rounded-full" style={{ width: `${rule.precision}%`, background: "#22D3EE" }} />
                    </div>
                    <span className="text-cyan-400 text-xs font-['JetBrains_Mono']">{rule.precision}%</span>
                  </div>
                ...
                {isExpanded && (
                  <div className="px-4 pb-4 pt-3" style={{ background: "rgba(34,211,238,0.02)", borderTop: "1px solid rgba(34,211,238,0.06)" }}>
                    <div className="grid grid-cols-4 gap-4 mb-3">
                      <div>
                        <p className="text-gray-500 text-xs uppercase tracking-wider font-['Inter'] mb-1">Condition Logic</p>
                        <p className="text-cyan-300 text-xs font-['JetBrains_Mono'] leading-relaxed">{rule.conditions}</p>
                      </div>
                      <div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="ml-auto flex items-center gap-2">
                        ...
                        <span className="text-cyan-400 text-xs font-['JetBrains_Mono'] font-bold">
                          {((2 * rule.precision * rule.recall) / (rule.precision + rule.recall)).toFixed(1)}%
                        </span>
                      </div>
                    </div>
                    {editingId === rule.id && (
                      <div className="mt-3 p-3 rounded-lg" style={{ background: "rgba(34,211,238,0.04)", border: "1px solid rgba(34,211,238,0.15)" }}>
                        <p className="text-cyan-400 text-xs font-semibold font-['Inter'] mb-2">Edit Rule Conditions</p>
                        <textarea defaultValue={rule.conditions} rows={2}
                          className="w-full px-3 py-2 rounded-lg text-xs outline-none resize-none"
                          style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(34,211,238,0.2)", color: "#22D3EE", fontFamily: "'JetBrains Mono', monospace" }} />
                        <div className="flex gap-2 mt-2">
                          <button className="px-3 py-1.5 rounded-lg text-xs font-semibold cursor-pointer whitespace-nowrap" style={{ background: "#22D3EE", color: "#060D1A" }}>Save Changes</button>
