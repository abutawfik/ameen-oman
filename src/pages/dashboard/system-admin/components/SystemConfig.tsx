...
const ConfigSection = ({ title, icon, children, badge, badgeColor }: SectionProps) => (
  <div className="rounded-xl p-5 mb-4" style={{ background: "rgba(10,37,64,0.8)", border: "1px solid rgba(184,138,60,0.12)" }}>
...
const SystemConfig = () => {
  ...
  const ConnStatus = ({ connKey }: { connKey: string }) => {
    ...
    if (!r) return null;
    return (
      <span className="flex items-center gap-1 text-xs font-['JetBrains_Mono']" style={{ color: r === "ok" ? "#4ADE80" : "#C94A5E" }}>
  ...
  return (
    <div className="space-y-0">
      ...
      <ConfigSection title="General Settings" icon="ri-settings-3-line">
        ...
        <div className="mt-3 flex items-center gap-3 p-3 rounded-lg" style={{ background: "rgba(184,138,60,0.05)", border: "1px solid rgba(184,138,60,0.1)" }}>
      ...
      {/* Security */}
      <ConfigSection title="Security Configuration" icon="ri-shield-keyhole-line" badge="CLASSIFIED" badgeColor="#C94A5E">
