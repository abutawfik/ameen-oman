...
const EVENT_CARDS = [
  { id: "boat" as EventType, icon: "ri-ship-line", label: "Boat Rental", labelAr: "تأجير قارب", desc: "Register boat rental with marine license, vessel details, passengers and GPS tracker", descAr: "تسجيل تأجير قارب مع رخصة الملاحة وبيانات السفينة والركاب وتتبع GPS", color: "#22D3EE", code: "MAR_BOAT" },
  ...
  { id: "diving" as EventType, icon: "ri-water-flash-line", label: "Diving Registration", labelAr: "تسجيل غوص", desc: "Register dive with certification level, site GPS, max depth and buddy details", descAr: "تسجيل الغوص مع مستوى الشهادة وإحداثيات الموقع وأقصى عمق وبيانات الرفيق", color: "#FB923C", code: "MAR_DIVING" },
];
      ...
      <header className="sticky top-0 z-40 flex items-center justify-between px-6 py-3 border-b" style={{ background: "rgba(6,13,26,0.95)", borderColor: "rgba(34,211,238,0.12)", backdropFilter: "blur(12px)" }}>
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 flex items-center justify-center rounded-lg" style={{ background: "rgba(34,211,238,0.1)", border: "1px solid rgba(34,211,238,0.2)" }}><i className="ri-anchor-line text-cyan-400 text-sm" /></div>
            <div>
              <div className="flex items-center gap-2">
                <span className="text-white font-bold text-sm">{isAr ? "أحداث البحرية" : "Marine Events"}</span>
                <span className="px-2 py-0.5 rounded-full text-xs font-bold" style={{ background: "rgba(34,211,238,0.12)", color: "#22D3EE", border: "1px solid rgba(34,211,238,0.2)" }}>AL-AMEEN Portal</span>
              </div>
        ...
        {!activeEvent && (<>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-8">
            {EVENT_CARDS.map(card => (
              <button key={card.id} type="button" onClick={() => handleSwitch(card.id)} className="group relative rounded-2xl border p-6 text-left cursor-pointer transition-all duration-300 flex flex-col gap-4" style={{ background: "rgba(10,22,40,0.8)", borderColor: "rgba(34,211,238,0.12)", backdropFilter: "blur(12px)" }} onMouseEnter={(e) => { const el = e.currentTarget as HTMLButtonElement; el.style.borderColor = `${card.color}50`; el.style.background = `${card.color}08`; el.style.transform = "translateY(-2px)"; }} onMouseLeave={(e) => { const el = e.currentTarget as HTMLButtonElement; el.style.borderColor = "rgba(34,211,238,0.12)"; el.style.background = "rgba(10,22,40,0.8)"; el.style.transform = "translateY(0)"; }}>
