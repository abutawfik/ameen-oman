  ...
  return (
    <div className="p-5 max-w-3xl">
      ...
      <div className="flex items-start gap-2 px-4 py-2.5 rounded-lg mb-5 text-xs" style={{ background: 'rgba(250,204,21,0.08)', border: '1px solid rgba(250,204,21,0.2)', color: '#FACC15' }}>
        ...
        <span>{isAr ? 'بيانات تغيير الغرفة تُرسل تلقائياً إلى منصة أمين.' : 'Room change data is automatically sent to AMEEN Platform.'}</span>
      </div>
      ...
      <div className="rounded-xl border border-cyan-500/20 p-5 mb-5 space-y-4" style={{ background: 'rgba(10,22,40,0.8)' }}>
        <h3 className="text-cyan-400 font-semibold text-sm uppercase tracking-wider">{isAr ? 'تفاصيل التغيير' : 'Change Details'}</h3>
        <div>
          <label className={labelCls}>{isAr ? 'الغرفة الجديدة' : 'New Room'}</label>
          <select value={newRoom} onChange={e => setNewRoom(e.target.value)} className={inputCls} style={{ background: 'rgba(10,22,40,0.9)' }}>
            <option value="" style={{ background: '#0A1628' }}>{isAr ? 'اختر غرفة' : 'Select new room'}</option>
            {rooms.filter(r => r.status === 'available').map(r => (
              <option key={r.id} value={r.number} style={{ background: '#0A1628' }}>{r.number} — {r.type} — Floor {r.floor} ({r.rateOMR} OMR)</option>
            ))}
          </select>
        </div>
      ...
      <div className="flex items-center justify-between pt-4 border-t border-cyan-500/10">
        <div className="flex items-center gap-2 text-xs" style={{ color: '#4ADE80' }}>
          ...
          {isAr ? 'سيتم إرسال بيانات التغيير إلى أمين تلقائياً' : 'Change data will auto-sync to AMEEN'}
        </div>
